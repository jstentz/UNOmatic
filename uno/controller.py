...
#   #  this should really be get action, which should encompass the functions below it, so we should really
#   # listen for all inputs at one time
#   # have to make it retry get_action upon return at the UNO level
#   # action class that is expandable that could be extended to include the website
#   # the controller needs to be 100% interruptable by the manager
#   def get_card(self, player: Player) -> Optional[Card]:
#     while (val := self.keypad_read()) not in [HWController.PLAY_TURN, HWController.SKIP_TURN]:
#       pass # TODO: add time.sleep()
